...
DROP TABLE if exists variazioneAnnuale;
...
SELECT D.ticker,D.sector, M.name, D.close, M.year
...
CREATE TABLE variazioneAnnuale AS
SELECT FC.ticker, FC.sector, FC.name, FC.year, round((LC.close/FC.close)*100-100,0) AS variazione
FROM firstClose FC JOIN lastClose LC ON(FC.ticker=LC.ticker AND FC.name=LC.name AND FC.year=LC.year AND FC.sector=LC.sector);
